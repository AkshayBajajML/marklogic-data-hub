...
const MatchingStepDetail: React.FC = () => {
    ...
    const [tableShow, toggleTableShow] = useState(true);
    ...
    const testMatchedUriColumns = [{
        ...
        render: (text,key) => (
         <div>
            <div className={styles.testMatchedHeading}><span onClick={(key) => {toggleShow(!show)
            toggleTableShow(true)}}><Icon className={styles.toggleIcon} type="down" rotate={show ? -90 : 0}/>
            <span className={styles.matchRulesetStyle}>{text.name} - {text.action}</span></span> (Threshold: {text.threshholdValue} )</div>
            {show=== false && <span><span onClick={(key) => {toggleTableShow(!tableShow)}}>
            <Icon className={styles.toggleTableViewIcon} type="down" rotate={tableShow ? -90 : 0}/></span>
            <span className={styles.testMatchedTableRow}>{text.entityNames[0]}</span>
            <span className={styles.rulesetColumn}>&nbsp;&nbsp;(Score: {text.score})</span>
            <MLButton type="primary" size="default" className={styles.compareButton}>Compare</MLButton><br />
            <span className={styles.matchedUriRow}>{text.entityNames[1]}</span>
            <ExpandableTableView key={key} rowData={text} tableShow={tableShow} allRuleset={curationOptions.activeStep.stepArtifact.matchRulesets}/></span>}
         </div>
        )
    }];
